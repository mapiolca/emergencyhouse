...
$registrationAvailable = $memberService->isReady((int) $conf->entity);
...
if ($action === 'register' && isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	$termsAccepted = !$termsPublished || GETPOSTINT('terms_accepted') > 0;
	$privacyAccepted = !$privacyPublished || GETPOSTINT('privacy_accepted') > 0;

	$identity = $emergencyhousePublicIp.'|'.EmergencyHouseEncryptionService::normalizeEmail($email);
	$rateLimitError = '';
	if (!$registrationAvailable) {
		$errorKey = 'PublicMemberRegistrationUnavailable';
	} elseif (!emergencyhousePublicConsumeRateLimit($db, (int) $conf->entity, 'register', $identity, 5, 3600, $rateLimitError)) {
		...
		if ($result <= 0) {
			...
			if ($memberService->provisionForAccount($account, $triggerUser) <= 0) {
				$db->rollback();
				$errorKey = 'ErrorMemberProvisioning';
			} else {
...
if (!$registrationAvailable) {
	emergencyhousePublicAlert('PublicMemberRegistrationUnavailable', 'warning');
} else {
	...
	print '<div class="eh-field"><label for="firstname">'.$langs->trans('Firstname').'</label><input id="firstname" name="firstname" required autocomplete="given-name" maxlength="50"></div>';
	print '<div class="eh-field"><label for="lastname">'.$langs->trans('Lastname').'</label><input id="lastname" name="lastname" required autocomplete="family-name" maxlength="50"></div>';
	...
	print '<div class="eh-field"><label for="phone">'.$langs->trans('Phone').'</label><input id="phone" type="tel" name="phone" autocomplete="tel" maxlength="30" inputmode="tel"><span class="eh-help">'.$langs->trans('PhoneInternationalFormatHelp').'</span></div>';
	...
	if ($privacyPublished) {
		$privacyLink = '<a href="'.dol_escape_htmltag(emergencyhousePublicUrl('privacy.php')).'" target="_blank" rel="noopener noreferrer">'
			.$langs->trans('PrivacyPolicy').'</a>';
		print '<label class="eh-switch"><span>'.$langs->trans('AcceptPrivacyPolicyPrefix').' '.$privacyLink.'.</span>';
		print '<input type="checkbox" role="switch" name="privacy_accepted" value="1" required></label>';
	}
	print '</div>';
